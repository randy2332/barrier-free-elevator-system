...
void childfunc() 
{
    ...
    while (1) 
    {
        ...
        //             printf("-----------------------------------------\n");
        //             fprintf(file1, "[Info] Passenger: %s, Need: From floor %hu to floor %hu , State: Finish !\n", user_info->name[i], shm_data[i]->start, shm_data[i]->destination);
...
void* thread_function(void* client_fd_ptr)
{
    ...
    while (1) 
    {
        ...
        if (bytes_read > 0) 
        {
            ...
            // while(1)
            // {
            //     pthread_mutex_lock(&open_close_mutex);
            //     int inside_count = 0;
            //     if(shm_flags[0])
            //     {   
            //         shm_flags[0] = 0;
            //         printf("Inside\n");
            //         pthread_mutex_unlock(&open_close_mutex);
            //     }
            //     if(shm_flags[1])
            //     {
            //         shm_flags[1] = 0;
            //         inside_count++;
            //         if(inside_count==1) printf("Arrive\n");
            //         pthread_mutex_unlock(&open_close_mutex);
            //         break;
            //     }
            // }
